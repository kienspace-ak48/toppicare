  ...
  return {
    ...
    CategoryAdd: async (req, res) => {
      try {
        ...
        if (!task1) {
          return res
            .status(400)
            .json({ success: false, mess: "Created failed" });
        }
        res.json({ success: true });
      } catch (error) {
        console.log(CNAME, error.message);
    ...
    CategoryDelete: async (req, res) => {
      try {
        ...
        if (!task1) {
          return res
            .status(400)
            .json({ success: false, mess: "delete is faield" });
        }
        res.json({ success: true });
      } catch (error) {
        console.log(CNAME, error.message);
    ...
    ArticleAdd: async (req, res) => {
      try {
        ...
        if (!flagCheck) {
          return res
            .status(400)
            .json({ success: false, mess: "input is not correct" });
        }
        const aDTO = {
          title,
          content,
          ...
          tags,
          categoryId: category,
          type: type === "0" ? "customer" : "technician",
        };
        console.log(aDTO);
    ...
    ArticleUpdate: async (req, res) => {
      try {
        ...
        console.log(_id);
        if (!_id) {
          return res.status(400).json({ success: false, mess: "lose id" });
        }
        const {
          category,
          desc,
        ...
        console.log(aDTO);
        const task1 = await articleService.update(aDTO, _id);
        if (!task1) {
          return res
            .status(400)
            .json({ success: false, mess: "update processing failed" });
        }
        res.json({ success: true });
      } catch (error) {
        console.log(CNAME, error.message);
    ...
    ArticleDelete: async (req, res) => {
      try {
        ...
        if (!_id)
          return res.status(400).json({ success: false, mess: "lose id" });
        const task1 = await articleService.delete(_id);
        if (!task1)
          return res
            .status(400)
            .json({ success: false, mess: "delete processing failed" });
        res.json({ success: true });
      } catch (error) {
        console.log(CNAME, error.message);
        res.status(500).json({ success: false, mess: "Server error" });
